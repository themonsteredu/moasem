...
type Program={zoom_join_url?:string|null;id:string;name:string;institution:{id:string;name:string}|null}
...
export default function AttendancePage(){
  ...
  const selection=`${programId}:${date}:${sessionType}`
  ...
  async function loadAttendance(){
    ...
    try{const response=await apiFetch(`/api/admin/attendance?program_id=${encodeURIComponent(programId)}&session_date=${date}&session_type=${sessionType}`,{headers,cache:'no-store'});const data=await response.json();if(!response.ok)throw new Error(data.error||'출석을 불러오지 못했습니다.');if(version!==requestVersion.current)return;setStudents(data.items??[]);setLoadedSelection(selection)}
  ...
  async function save(){
    ...
    try{const response=await apiFetch('/api/admin/attendance',{method:'POST',headers,body:JSON.stringify({program_id:programId,session_date:date,session_type:sessionType,records:students.map(student=>({student_id:student.id,status:student.status,note:student.note}))})});const data=await response.json();if(!response.ok)throw new Error(data.error||'저장하지 못했습니다.');setMessage('출석을 저장했습니다.')}
  ...
  async function saveZoomLink(){
    ...
    setSaving(true);setMessage('')
    try{const response=await apiFetch('/api/admin/programs',{method:'PATCH',headers,body:JSON.stringify({program_id:programId,zoom_join_url:zoomLink.trim()})});const data=await response.json();if(!response.ok)throw new Error(data.error);setPrograms(items=>items.map(item=>item.id===programId?{...item,zoom_join_url:data.item.zoom_join_url}:item));setMessage('줌 링크를 저장했습니다.')}
    catch(error){setMessage(error instanceof Error?error.message:'줌 링크를 저장하지 못했습니다.')}
    finally{setSaving(false)}
  }
  const selectedProgram=programs.find(item=>item.id===programId)
  return <Workspace current="/attendance" title="수업·출석" description="수업일의 학생 명단을 확인하고 출석을 기록하세요.">
    <StaffAccess onLoad={loadPrograms} busy={busy||saving}/>
    <section className="surface">
      <div className="toolbar">
        <label className="field"><span>프로그램</span><select value={programId} disabled={saving} onChange={event=>{clearSelection();setProgramId(event.target.value);setZoomLink(programs.find(item=>item.id===event.target.value)?.zoom_join_url??'')}}><option value="">프로그램 선택</option>{programs.map(program=><option key={program.id} value={program.id}>{program.institution?.name} · {program.name}</option>)}</select></label>
        <label className="field"><span>수업 방식</span><select aria-label="수업 방식" value={sessionType} disabled={saving} onChange={event=>{clearSelection();setSessionType(event.target.value)}}><option value="in_person">대면</option><option value="zoom">줌</option></select></label>
      ...
      {programId&&<div className="surface" style={{marginBottom:24}}><h2>줌 수업</h2><p>Zoom에서 복사한 참가자 초대 링크를 등록하세요. 강사는 Zoom에서 호스트 계정으로 로그인해 수업을 시작해 주세요.</p><label className="field"><span>줌 초대 링크</span><input type="url" value={zoomLink} disabled={saving} placeholder="https://…zoom.us/j/…?pwd=…" onChange={event=>setZoomLink(event.target.value)}/></label><div className="toolbar"><button className="button button-dark" disabled={saving} onClick={saveZoomLink}>링크 저장</button>{selectedProgram?.zoom_join_url&&<a className="button button-primary" href={selectedProgram.zoom_join_url} target="_blank" rel="noopener noreferrer">줌 수업 입장</a>}</div><p className="meta">무료 Zoom 계정은 회의당 최대 40분입니다. 입장 버튼은 출석을 기록하지 않습니다. 줌 수업 출석은 아래에서 직접 확인하고 저장해 주세요.</p></div>}
