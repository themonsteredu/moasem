...
export default function AttendancePage(){
  ...
  async function loadAttendance(){
    ...
    setStudents(data.items??[]); setMessage('')
  }
  function setStatus(id:string,status:string){setStudents(items=>items.map(x=>x.id===id?{...x,status}:x))}
  async function save(){
    const res=await fetch('/api/admin/attendance',{method:'POST',headers,body:JSON.stringify({program_id:programId,session_date:date,records:students.map(x=>({student_id:x.id,status:x.status,note:x.note}))})})
  ...
  return <main style={{maxWidth:1100,margin:'0 auto',padding:32,fontFamily:'Arial, Apple SD Gothic Neo, sans-serif'}}>
    ...
    <section style={{background:'#fff',border:'1px solid #e5e7eb',borderRadius:16,overflow:'hidden'}}>
      <div style={{display:'flex',gap:10,padding:18,borderBottom:'1px solid #eee',flexWrap:'wrap'}}>
        <select value={programId} onChange={e=>setProgramId(e.target.value)} style={{padding:10,border:'1px solid #ddd',borderRadius:10,minWidth:280}}><option value="">프로그램 선택</option>{programs.map(p=><option key={p.id} value={p.id}>{p.institution?.name} · {p.name}</option>)}</select>
        <input type="date" value={date} onChange={e=>setDate(e.target.value)} style={{padding:10,border:'1px solid #ddd',borderRadius:10}}/>
      ...
      <div style={{padding:18,textAlign:'right'}}><button onClick={save} disabled={!students.length} style={{padding:'11px 18px',border:0,borderRadius:10,background:'#f26522',color:'#fff',fontWeight:700}}>출석 저장</button></div>
    </section>
