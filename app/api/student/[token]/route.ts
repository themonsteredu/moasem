...
export async function GET(_req:NextRequest,{params}:{params:{token:string}}){try{
 const s=await portalStudent(params.token),db=getSupabaseAdmin()
...
export async function POST(req:NextRequest,{params}:{params:{token:string}}){try{
 assertSameOrigin(req);const s=await portalStudent(params.token),b=await req.json(),url=textInput(b.url,2000);const videos=await studentVideos(s.id,s.program_id);if(!videos.some(v=>v.url===url))throw new AccessError(400,'등록된 영상만 확인할 수 있어요.')
 await operate(null,s.id,'video',{url,confirmed:b.confirmed===true},params.token);return NextResponse.json({ok:true},{headers:privateHeaders})
