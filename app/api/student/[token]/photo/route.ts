...
export async function POST(req:NextRequest,{ params }: { params: Promise<{ token: string }> }){try{
 assertSameOrigin(req);const s=await portalStudent((await params).token)
 ...
 try { await operate(null,s.id,'photo',{id,homework_id:homeworkId,path},(await params).token) } catch(e) { const check=await db.from('homework_photos').select('id').eq('id',id).maybeSingle();if(!check.error&&!check.data)await db.storage.from('moasem-homework').remove([path]);throw e }
