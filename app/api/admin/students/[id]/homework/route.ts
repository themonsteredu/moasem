...
export async function GET(req:NextRequest,{params}:{params:{id:string}}){try{
 const staff=await assertStaff(req),s=await staffStudent(staff,params.id),db=getSupabaseAdmin()
...
export async function POST(req:NextRequest,{params}:{params:{id:string}}){try{
 const staff=await assertStaff(req),s=await staffStudent(staff,params.id),body=await req.json();let payload:any={}
