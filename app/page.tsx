...
export default function Home(){
  ...
  async function submit(event:FormEvent<HTMLFormElement>){
    ...
    try{
      const response=await fetch(`/api/admin/${tab}`,{method:'POST',headers,body:JSON.stringify(payload)})
    ...
    <div className="management-grid">
      <section className="surface list-surface">
        ...
        <div id="registration-list" role="tabpanel" aria-labelledby={`tab-${tab}`}>
          {!loaded?<EmptyState title="운영 현황을 불러오세요" description="관리 키로 연결하면 등록된 기관과 학생을 확인할 수 있습니다."/>:counts[tab]===0?<EmptyState title={`아직 등록된 ${names[tab]}이 없습니다`} description={tab==='institutions'?'함께 수업할 기관의 이름과 담당자를 먼저 등록해 주세요.':tab==='programs'?'기관을 선택하고 수업 기간과 담당 강사를 등록해 주세요.':'프로그램을 선택하고 학생과 보호자 정보를 등록해 주세요.'} icon={tab==='students'?'people':'overview'}/>:<div className="table-scroll">
