...
type Student={is_under_14:boolean|null;consent:ConsentSummary;id:string;name:string;grade:number;student_number:string|null;program:{id:string;name:string;institution:{id:string;name:string}|null}|null;guardian:{id:string;name:string|null;phone:string;language:string}|null}
...
export default function Home(){
  ...
  async function submit(event:FormEvent<HTMLFormElement>){
    ...
    if(tab==='students'){payload.grade=Number(payload.grade);payload.is_under_14=payload.is_under_14==='unknown'?null:payload.is_under_14==='true'}
    ...
    try{
      ...
      form.reset()
      await loadAll()
      if(tab==='students'){setConsentStudentId(data.item.id);setMessage(payload.is_under_14===false?'학생 정보를 저장했습니다.':'학생 정보를 저장했습니다. 연령 확인과 보호자 동의를 이어서 확인해 주세요.')}else setMessage(`${names[tab]} 등록을 완료했습니다.`)
  ...
  return <Workspace current="/" title="기관·학생 관리" description="함께하는 기관과 아이들의 수업을 관리하세요." action={<span className="heading-tag">운영 현황</span>}>
    <StaffAccess onLoad={loadAll} busy={busy||saving||consentBusy}/>
    <section className="summary-strip" aria-label="등록 현황">
      {(['institutions','programs','students'] as Tab[]).map((key,index)=><button key={key} className={tab===key?'summary-item selected':'summary-item'} onClick={()=>setTab(key)} aria-pressed={tab===key} disabled={saving||consentBusy}><span className="summary-label">{['함께하는 기관','운영 프로그램','등록 학생'][index]}<Icon name="arrow" size={17}/></span><strong>{loaded?counts[key]:'—'}<small>{index===2?'명':'개'}</small></strong><span className="summary-note">{loaded?`${names[key]} 목록 보기`:'목록을 불러오면 표시됩니다'}</span></button>)}
    </section>
    <div className="management-grid">
      <section className="surface list-surface">
        ...
        <div id="registration-list" role="tabpanel" aria-labelledby={`tab-${tab}`}>
          {!loaded?<EmptyState title="운영 현황을 불러오세요" description="새로고침을 누르면 등록된 기관과 학생을 불러옵니다."/>:counts[tab]===0?<EmptyState title={`아직 등록된 ${names[tab]}이 없습니다`} description={tab==='institutions'?'함께 수업할 기관의 이름과 담당자를 먼저 등록해 주세요.':tab==='programs'?'기관을 선택하고 수업 기간과 담당 강사를 등록해 주세요.':'프로그램을 선택하고 학생과 보호자 정보를 등록해 주세요.'} icon={tab==='students'?'people':'overview'}/>:<div className="table-scroll">
            ...
            {tab==='students'&&<table className="data-table"><thead><tr><th>학생</th><th>프로그램·기관</th><th>보호자</th><th>안내 언어</th><th>보호자 동의</th></tr></thead><tbody>{students.map(item=><tr key={item.id}><td><a className="table-name student-record-link" href={`/students/${item.id}`}>{item.name}</a><small>{item.grade}학년{item.student_number?` · ${item.student_number}`:''}</small></td><td>{item.program?.name||'—'}<small>{item.program?.institution?.name||'—'}</small></td><td>{item.guardian?.name||'보호자'}<small>{item.guardian?.phone||'—'}</small></td><td><span className="badge">{languageLabel[item.guardian?.language||'ko']}</span></td><td><ConsentBadge student={item}/><button className="text-button" disabled={consentBusy} onClick={()=>setConsentStudentId(item.id)}>동의 관리</button></td></tr>)}</tbody></table>}
          </div>}
        </div>
      ...
      <aside className="surface editor-surface">
        ...
        <form className="editor-form" key={tab} onSubmit={submit}>
          <fieldset disabled={saving||consentBusy}>
          ...
          {tab==='students'&&<><Field title="프로그램 *"><select name="program_id" required><option value="">프로그램 선택</option>{programs.map(item=><option key={item.id} value={item.id}>{item.institution?.name} · {item.name}</option>)}</select></Field><div className="form-columns"><Field title="학생 이름 *"><input name="name" required/></Field><Field title="학년 *"><select name="grade" required>{Array.from({length:12},(_,index)=>index+1).map(grade=><option key={grade} value={grade}>{grade}학년</option>)}</select></Field></div><Field title="만 14세 미만 여부 *"><select name="is_under_14" required defaultValue=""><option value="" disabled>확인한 내용 선택</option><option value="true">예 · 만 14세 미만</option><option value="false">아니오 · 만 14세 이상</option><option value="unknown">아직 확인 필요</option></select></Field><Field title="학생 번호"><input name="student_number" placeholder="선택 입력"/></Field><div className="form-divider">보호자 안내 정보</div><Field title="보호자 이름"><input name="guardian_name"/></Field><Field title="보호자 연락처 *"><input name="guardian_phone" type="tel" required placeholder="010-0000-0000"/></Field><Field title="안내 언어 *"><select name="guardian_language" defaultValue="ko"><option value="ko">한국어</option><option value="en">영어</option><option value="vi">베트남어</option><option value="zh-CN">중국어 간체</option></select></Field></>}
          </fieldset>
          <button className="button button-primary full-width" disabled={saving||busy||consentBusy||!loaded}><Icon name="plus" size={17}/>{saving?'저장 중…':`${names[tab]} 등록`}</button>
        </form>
      </aside>
    </div>
    {consentStudent&&<StudentConsent key={consentStudent.id} student={consentStudent} onClose={()=>setConsentStudentId('')} onChanged={loadAll} onBusy={setConsentBusy}/>}
