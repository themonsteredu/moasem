...
export default function InstitutionPage({params}:{params:{token:string}}){
  ...
  return <main className="institution-page">
    ...
    <section><h2>02. 학생별 학습 현황</h2>{!students.length?<EmptyState title="등록된 학생이 없습니다" description="학생이 등록되면 출석 현황을 확인할 수 있습니다." icon="people"/>:<div className="table-scroll"><table className="data-table"><thead><tr><th>학생</th><th>학년</th><th>프로그램</th><th>대면 출석률</th><th>줌 출석률</th><th>이번 주 과제</th><th>오답 요약</th></tr></thead><tbody>{students.map(student=><tr key={student.id}><td><span className="table-name">{student.name}</span></td><td>{student.grade}학년</td><td>{programs.find(program=>program.id===student.program_id)?.name||'—'}</td><td>{rate(student.id)}</td><td>{rate(student.id,'zoom')}</td><td className="meta">연결 예정</td><td className="meta">연결 예정</td></tr>)}</tbody></table></div>}<p className="institution-footnote">출석률은 강사가 직접 저장한 출석 기록 기준이며, 지각을 출석에 포함합니다. 과제 제출과 오답 요약은 연결 후 표시됩니다.</p></section>
