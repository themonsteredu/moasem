...
type Program={zoom_join_url?:string|null;id:string;name:string;starts_on:string;ends_on:string;status:string}
...
export default function InstitutionPage(){
  ...
  return <main className="institution-page">
    ...
    <section><h2>01. 프로그램 현황</h2>{!programs.length?<EmptyState title="등록된 프로그램이 없습니다" description="프로그램이 등록되면 수업 기간과 참여 학생 수가 표시됩니다."/>:<div className="table-scroll"><table className="data-table"><thead><tr><th>프로그램</th><th>운영 기간</th><th>학생 수</th><th>상태</th><th>줌 수업</th></tr></thead><tbody>{programs.map(program=><tr key={program.id}><td><span className="table-name">{program.name}</span></td><td>{program.starts_on} ~ {program.ends_on}</td><td>{students.filter(student=>student.program_id===program.id).length}명</td><td><span className={`badge ${program.status==='active'?'badge-green':''}`}>{statusLabel[program.status]||program.status}</span></td><td>{program.zoom_join_url?<a className="button button-dark" href={program.zoom_join_url} target="_blank" rel="noopener noreferrer">줌 수업 입장</a>:<span className="meta">링크 미등록</span>}</td></tr>)}</tbody></table></div>}</section>
    <section><h2>02. 학생별 학습 현황</h2>{week&&<p>과제·오답 기준: {week.start} ~ {week.end}</p>}{!students.length?<EmptyState title="등록된 학생이 없습니다" description="학생이 등록되면 출석 현황을 확인할 수 있습니다." icon="people"/>:<div className="table-scroll"><table className="data-table"><thead><tr><th>학생</th><th>학년</th><th>프로그램</th><th>대면 출석률</th><th>줌 출석률</th><th>이번 주 과제</th><th>오답 요약</th></tr></thead><tbody>{students.map(student=><tr key={student.id}><td><span className="table-name">{student.name}</span></td><td>{student.grade}학년</td><td>{programs.find(program=>program.id===student.program_id)?.name||'—'}</td><td>{rate(student.id)}</td><td>{rate(student.id,'zoom')}</td><td>{(()=>{const x=learning.find(l=>l.student_id===student.id);return x?.total?`${x.submitted}/${x.total} 제출`:'과제 없음'})()}</td><td>{(()=>{const x=learning.find(l=>l.student_id===student.id);return x?.wrong?.length?x.wrong.map((t:any)=>`${t.name} ${t.count}회`).join(' · '):x?.legacy_summary||'기록 없음'})()}</td></tr>)}</tbody></table></div>}<p className="institution-footnote">출석률은 강사가 직접 저장한 출석 기록 기준이며, 지각을 출석에 포함합니다. 과제는 이번 주 마감 기준이며 오답은 이번 주 수업 기록입니다.</p></section>
